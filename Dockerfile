...
# Bedtools2
RUN DEBIAN_FRONTEND=noninteractive wget "https://github.com/arq5x/bedtools2/releases/download/v2.24.0/bedtools-2.24.0.tar.gz" && tar -xzf bedtools-2.24.0.tar.gz && cd bedtools2 && make && make install && cd ..

# Bowtie
RUN DEBIAN_FRONTEND=noninteractive wget "http://downloads.sourceforge.net/project/bowtie-bio/bowtie/1.1.0/bowtie-1.1.0-src.zip" && unzip bowtie-1.1.0-src.zip && cd bowtie-1.1.0 && make && ln bowtie /usr/local/bin/ && ln bowtie-* /usr/local/bin/ && cd ..

# Bowtie2
RUN DEBIAN_FRONTEND=noninteractive wget "http://downloads.sourceforge.net/project/bowtie-bio/bowtie2/2.2.3/bowtie2-2.2.3-source.zip" && unzip bowtie2-2.2.3-source.zip && cd bowtie2-2.2.3 && make && ln bowtie2 /usr/local/bin/ && ln bowtie2-* /usr/local/bin/ && cd ..

# BWA
RUN DEBIAN_FRONTEND=noninteractive wget "http://downloads.sourceforge.net/project/bio-bwa/bwa-0.7.12.tar.bz2" && tar -xjf bwa-0.7.12.tar.bz2 && cd bwa-0.7.12 && make && ln bwa /usr/local/bin && cd ..

# Flux Simulator (requires default-jre)
RUN DEBIAN_FRONTEND=noninteractive wget "http://sammeth.net/artifactory/barna/barna/barna.simulator/1.2.1/flux-simulator-1.2.1.tgz" && tar -xzf flux-simulator-1.2.1.tgz && cd flux-simulator-1.2.1/bin && ln flux-simulator /usr/local/bin/ && cd ../lib && ln *.jar /usr/local/lib/ && cd ../..

# GSNAP
RUN DEBIAN_FRONTEND=noninteractive wget "http://research-pub.gene.com/gmap/src/gmap-gsnap-2014-12-29.tar.gz" && tar -xzf gmap-gsnap-2014-12-29.tar.gz && cd gmap-2014-12-29 && ./configure --prefix=/usr/local/ && make && make install && cd ..
