...
# Bowtie
RUN DEBIAN_FRONTEND=noninteractive wget "http://softlayer-dal.dl.sourceforge.net/project/bowtie-bio/bowtie/1.1.0/bowtie-1.1.0-src.zip" && unzip bowtie-1.1.0-src.zip && cd bowtie-1.1.0 && make && ln bowtie /usr/local/bin/ && ln bowtie-* /usr/local/bin/ && cd ..

# Bowtie2
RUN DEBIAN_FRONTEND=noninteractive wget "http://softlayer-dal.dl.sourceforge.net/project/bowtie-bio/bowtie2/2.2.3/bowtie2-2.2.3-source.zip" && unzip bowtie2-2.2.3-source.zip && cd bowtie2-2.2.3 && make && ln bowtie2 /usr/local/bin/ && ln bowtie2-* /usr/local/bin/ && cd ..

# BWA
RUN DEBIAN_FRONTEND=noninteractive wget "http://tcpdiag.dl.sourceforge.net/project/bio-bwa/bwa-0.7.12.tar.bz2" && tar -xjf bwa-0.7.12.tar.bz2 && cd bwa-0.7.12 && make && ln bwa /usr/local/bin && cd ..
